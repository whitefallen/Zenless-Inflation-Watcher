...
import { Avatar as AvatarType, Buddy, DeadlyAssaultRun } from "@/types/deadly-assault"
import Image from "next/image"

interface TeamsOverviewProps {
  floors: DeadlyAssaultRun[]
}
...
export function TeamsOverview({ floors }: TeamsOverviewProps) {
  ...
  if (floors) {
    floors.forEach(run => {
      run.avatar_list.forEach(avatar => {
        if (!uniqueCharacters.has(avatar.id)) {
          ...
          if (char) {
            char.usageCount++;
            char.floors.push(runIndex);
          }
        }
      });

      const buddy = run.buddy;
